...
class CrossAttention(nn.Module):
    def __init__(self, dim, mode, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., length=27):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
    ...
    def forward(self, x):
        """
        Cross-Attention between RGB and NIR features.
        Args:
            x: x[x_rgb, x_nir] B, N + N, C
        ...
        B, N, C = x.shape
        lens_x = 256
        if self.mode == 'rgb2nir':
            q = x[:, :lens_x, :]  # x_rgb
            k = x[:, lens_x:, :]  # x_nir
            v = x[:, lens_x:, :]
        elif self.mode == 'nir2rgb':
            q = x[:, lens_x:, :]  # x_nir
            k = x[:, :lens_x, :]  # x_rgb
            v = x[:, :lens_x, :]
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, lens_x, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        if self.mode == 'rgb2nir':
