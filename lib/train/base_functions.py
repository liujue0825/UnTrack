...
# datasets related
from lib.train.dataset import Lasot, Got10k, MSCOCOSeq, ImagenetVID, TrackingNet, COMTB_trainingSet, COMTB_testingSet
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["COMTB_trainingSet", "COMTB_testingSet", "LASOT", "GOT10K_vottrain", "GOT10K_votval",
                        "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET"]
        # Cross-Modality
        if name == "COMTB_trainingSet":
            datasets.append(
                COMTB_trainingSet(settings.env.comtb_trainingset_dir, split='train', image_loader=image_loader))

        if name == "COMTB_testingSet":
            datasets.append(COMTB_testingSet(settings.env.comtb_testingset_dir, image_loader=image_loader))
